...
- [ ] 参加者向けフィードバックを Cloud Run へ反映して、本番で通し確認する
  ...
  - 現在の既知不具合:
    - 管理画面では `80.0 pts` なのに、ゲスト向けフィードバックでは `7.9 pts` と返る事例がある
    - 10 分の 1 っぽい値になっているため、表示スコア計算まわりのバグを疑う
    - 本番前に必ず修正・再確認したい

## 次にやる順番

1. 参加者向けフィードバックを Cloud Run に反映して本番確認
2. ゲスト向けフィードバックの点数バグ修正
3. Gemini prompt の改善継続
4. presentation の最後の微調整
5. ネットワーク不調時の救済フロー確認
