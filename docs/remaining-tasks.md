...
- [ ] presentation の最後の微調整を 1 回だけやる
  ...
  - 他タスク完了後に最後にまとめてやる
  - 名前表示のフォント課題:
    - `髙𫞎` のような名前で、カタカナ / 通常漢字 / 異字体 `𫞎` が別々のフォントに fallback して見えてしまう
    - 名前の一部だけではなく、`髙𫞎 さん` のような表示全体を1つの統一フォントで見せたい
    - 意図としては、`𫞎` に対応しているフォントに他の文字も合わせる形にしたい
...
- [ ] 参加者向けフィードバックを Cloud Run へ反映して、本番で通し確認する
  ...
  - 3 位までが片側だけにならないよう自動調整が効くこと
  - presentation の公開リンクで `feedback_released` が切り替わること
  - `/join/...` で result 発表前は見えず、公開後に見えること
  - 5 項目点数 + コメント構成が想定どおり見えること
  - 対象外の人の表示スコアが 3 位未満に収まること
  - 以前あった `80.0 pts -> 7.9 pts` のようなズレが再現しないこと
  - ローカルでは現在そのズレは再現していないため、Cloud Run 上でも再確認する

## 次にやる順番

1. 参加者向けフィードバックを Cloud Run に反映して本番確認
2. Gemini prompt の改善継続
