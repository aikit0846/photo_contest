...
- [x] `feedback_released` と guest 画面の表示条件を、実機で再確認した
  ...
  - さらに採点開始時にも `feedback_released=false` へ戻す実装を追加し、公開状態にならないことを確認済み
...
- [x] guest 導線の UI 改善を入れた
  - `新郎友人 / 新郎親族 / 新婦友人 / 新婦親族` のボタン色を分けた
  - 名前選択画面に `(五十音順)` を追加した
  - プレゼン画面に、目立たない管理画面戻り導線を追加した
...
- [x] Cloud Tasks + job 方式で、開始端末から切り離した採点ができることを確認した
  - PC から採点開始するパターンを確認済み
...
- [ ] Cloud Tasks + job 方式の進捗表示 / エラーメッセージ UX を整える
  ...
  - ただし backend 側の採点自体は継続し、最終的に `70` 件すべて完了した
  - `10` 件 Gemini テストでは、iPhone 側の失敗系 UI は再現しなかった
  - iPhone 側では進捗表示が出ることを確認できた
  - ただし MacBook Air 側は、タブを更新しないと進捗表示や採点完了数が更新されない事象が残っている
  - 監視端末の自動更新 UX は、本番前に改善余地あり

- [ ] presentation 画面に、名前以外の所属情報も表示する
  - 名前だけでなく `新郎友人 / 新婦友人 / 新郎親族 / 新婦親族` などの情報も出せるとよさそう
  - 可能なら卓情報も出したい
  - 優先度は低め
