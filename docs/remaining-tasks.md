...
- [ ] presentation の最後の微調整を 1 回だけやる
  ...
  - 他タスク完了後に最後にまとめてやる
  - 現時点の気づき:
    - ゲスト名に `髙𫞎` のような異字体がある場合、`𫞎` が現在のフォントで対応しておらず別フォント表示になる
    - 優先度は高くないが、本番前に font fallback は見直したい

- [ ] 管理画面の投稿一覧で、その投稿がランキング対象外かどうかを一目で分かるようにする
  - 現状は全ての投稿に `ランキング対象外にする` ボタンが出るため、状態が分かりにくい
  - ボタンだけでなく現在状態も明示したい

- [ ] 管理画面の `表示順位` 機能を削除する
  - 2位を1位にしても順位が入れ替わらないバグがある
  - ただし、そもそもこの機能は不要
  - 順位を動かしたいときは `点数補正` で対応する運用に寄せる
  - UI / ルータ / repository から整理して外したい
...
2. 投稿一覧のランキング対象外状態表示を改善
3. `表示順位` 機能を削除
4. 必要なら `gemini-2.5-flash-lite` と比較する
5. ネットワーク不調時の救済フロー確認
