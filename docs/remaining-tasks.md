...
- [ ] 管理画面の投稿一覧で、フィードバック用コメント全文を表示する
  - 現在は AI コメントの最初の 1 文だけが見える
  - join 画面などで実際に返るコメント全文を管理画面でも見えるようにしたい
...
1. `feedback_released` と guest 画面の表示条件を、実機で再確認する
